...
public class AppAuthenticationStateProvider : AuthenticationStateProvider
{
    ...
    public override async Task<AuthenticationState> GetAuthenticationStateAsync()
    {
        try
        {
            string savedToken = await _localStorageService.GetItemAsync<string>(LocalStorageBearerKeyName);
            ...
            {
                await _localStorageService.RemoveItemAsync(LocalStorageBearerKeyName);
                return new AuthenticationState(new ClaimsPrincipal(new ClaimsIdentity()));
            }

            // Get claims from token and build an authentication user object
